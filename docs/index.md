...
- **Inspectable trust surface.** No external Swift package
  ...
  published test vectors. You can audit the trust boundary without
  trusting a transitive dependency graph.
...
The v1.0 trust story is the inspectable trust surface itself: open
source under MIT, no external Swift package dependencies in the
crypto path, vendored Argon2 at a fixed commit, and zero network
calls from the cryptographic or database-handling code. A formal
third-party audit is planned for v2.0; the report will publish
...
- The cryptographic surface is small and inspectable. There are no
  ...
  vendored Argon2 at a fixed commit.
- The codebase makes zero network calls from the crypto and
  database-handling paths. This is a property you can verify by
  reading the source, and (separately) by running the app under
  network sandboxing.
- Apple's notarization and code signing prevent silent post-install
  modification — but not a compromise at build time.

If your threat model can't accept "trust the App Store binary," the
right move is to build from source yourself and sideload via Xcode.
